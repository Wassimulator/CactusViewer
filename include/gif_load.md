...
proxy that is discarded after every call):
  * `GIF_WHDR::xdim` - global GIF width, always constant across frames
                       (further referred to as 'always constant across frames'); [0; 65535]
  * `GIF_WHDR::ydim` - global GIF height, always constant across frames; [0; 65535]
  * `GIF_WHDR::clrs` - number of colors in the current palette (local palettes
                       ...
                       referred to as 'may vary across frames'); {2; 4; 8; 16; 32; 64; 128; 256}
  * `GIF_WHDR::bkgd` - 0-based background color index for the current palette,
                       always constant across frames (sic always constant across frames, as this index is set globally)
  * `GIF_WHDR::tran` - 0-based transparent color index for the current palette
                       (or −1 when transparency is disabled), may vary across frames
  * `GIF_WHDR::intr` - boolean flag indicating whether the current frame is
                  [interlaced](https://en.wikipedia.org/wiki/GIF#Interlacing);
                       ...
                       below), may vary across frames
  * `GIF_WHDR::mode` - next frame (sic next, not current) blending mode, may vary across frames:
  ...
  * `GIF_WHDR::frxd` - current frame width, may vary across frames; [0; 65535]
  * `GIF_WHDR::fryd` - current frame height, may vary across frames; [0; 65535]
  * `GIF_WHDR::frxo` - current frame horizontal offset, may vary across frames; [0; 65535]
  * `GIF_WHDR::fryo` - current frame vertical offset, may vary across frames; [0; 65535]
  * `GIF_WHDR::time` - next frame delay in GIF time units (1 unit = 10 msec),
                       may vary across frames; negative values are possible here, they mean
  ...
  * `GIF_WHDR::ifrm` - 0-based index of the current frame, always varies
                       across frames (further referred to as 'always varies across frames')
  * `GIF_WHDR::nfrm` - total frame count, negative if the GIF data supplied
                       is incomplete, always constant across frames during a single `GIF_Load()` call
                       but may vary across `GIF_Load()` calls
  * `GIF_WHDR::bptr` - [frame writer:] pixel indices for the current frame,
                       always constant across frames (it is only the pointer address that is constant;
                       the pixel indices stored inside = may vary across frames); [metadata
